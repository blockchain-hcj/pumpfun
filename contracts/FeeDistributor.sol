...
contract FeeDistributor is IFeeDistributor, Ownable, ReentrancyGuard {
    ...
    constructor(IVotingEscrow votingEscrow, uint256 startTime) Ownable(msg.sender){
    ...
    function _checkpointUserBalance(address user) internal {
        ...
        for (uint256 i = 0; i < 50; ++i) {
            if (nextWeekToCheckpoint >= nextUserPoint.ts && userEpoch <= maxUserEpoch) {
                ...
                uint256 userBalance = currentUserPoint.bias > currentUserPoint.slope * dt ? uint256(uint128(currentUserPoint.bias - currentUserPoint.slope * dt)) : 0;
    ...
    function _checkpointTotalSupply() internal {
        ...
        for (uint256 i = 0; i < 20; ++i) {
            ...
            int128 dt = nextWeekToCheckpoint > pt.ts ? int128(int256(nextWeekToCheckpoint - pt.ts)) : int128(0);
            int128 supply = pt.bias - pt.slope * dt;
            _veSupplyCache[nextWeekToCheckpoint] = supply > 0 ? uint256(uint128(supply)) : 0;
            // This is safe as we're incrementing a timestamp
            nextWeekToCheckpoint += 1 weeks;
        }
